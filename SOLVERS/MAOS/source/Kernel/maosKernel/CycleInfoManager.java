...
public final class CycleInfoManager implements ICycleInfoEngine, ITrialInitEngine {
  private final String RUN_TIME_FLAG = "initTime";//"runTime";
...
//    currCycleResult.submitData(giniIndexAnalysizer.getName(), "N/A");
  ...
  public void setStart() {
    ...
    prev_time = 0;
    GlobalTools.CPUTimeCostCounter.setStart("prev_time");
...
//    }
    //GradedOut.showIMPORTANTMessage("prev_time: " + currCycleResult.Time - prev_time);
    if (currCycleResult.Time - prev_time > 600){
      System.exit(0);
    }
    if (isImproved) {
      currCycleResult.BState.importEncodeState(cycleBestState);
      bestCycleResult.importData(currCycleResult);
      System.out.print((int)bestCycleResult.BState.getEncodeInfo()+" ");
      System.out.println(bestCycleResult.Time);
      prev_time = bestCycleResult.Time;

      //GlobalTools.CPUTimeCostCounter.setEnd("prev_time");
      //GradedOut.showIMPORTANTMessage("prev_time: " + GlobalTools.CPUTimeCostCounter.getTotalTimeInSeconds("prev_time"));
      //GlobalTools.CPUTimeCostCounter.reset("prev_time");
      //GlobalTools.CPUTimeCostCounter.setStart("prev_time");
    }
